...
const calculateLapsedTime = (initialLapsedTime, startTime, stoppedPeriods) => Math.floor((initialLapsedTime + Date.now() - startTime - stoppedPeriods.reduce((ac, cur) => ac + cur, 0)) / 100) / 10;
export default ({initialValue = 0}: UseTimerProps): RenderHookResult => {
  ...
  const [stoppedPeriods, setStoppedPeriods] = useState([]);
